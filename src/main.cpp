...
// ─── Application States ────────────────────────────────────────
enum ApplicationState { STATE_BOOT, STATE_IDLE, STATE_UPDATING };

ApplicationState currentState;
uint32_t stateEnteredAt;

// Reads the pin, Debounces, Tracks time, Emits one event max per call. Need to
// flesh out function definition. ButtonEvent updateButton(Button& button,
// uint32_t now);
...
// void loop() {
//   ButtonEvent event = updateButton(sleepButton, millis());
